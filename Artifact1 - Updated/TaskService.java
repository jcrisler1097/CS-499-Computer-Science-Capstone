...
public class TaskService {
	...
	public static String taskServiceID; //Creates taskServiceID variable
	public String taskServiceName; //Creates taskServiceName variable
	public String taskServiceDescription; //Creates taskServiceDescription variable
	...
	private static void checkDuplicateEntry(String ID) {
	    for (Task task : taskList) {
	        if (task.getTaskID().equals(ID)) {
	            throw new IllegalArgumentException("Error: Duplicate task ID found: " + ID);
	        }
	    }
	}
	...
	public static void addNewTask(String name, String description) {
		...
		//Calls checkDupleicateEntry method to check for duplicate entries before creating new task
	...
	//Search method for testing to search by name
	public static int searchTaskListName(String uniqueTaskName) {
		int searchResult = 0; //Initializes searchResult to 0
		
		for (int i = 0; i < taskList.size(); i++) {
			if (uniqueTaskName.compareTo(taskList.get(i).getTaskName()) == 0) {
				searchResult = 1;
			}
	...
	//Search method for testing to search by description
	public static int searchTaskListDescription(String uniqueTaskDescription) {
		int searchResult = 0; //Initializes searchResult to 0
		
		for (int i = 0; i < taskList.size(); i++) {
			if (uniqueTaskDescription.compareTo(taskList.get(i).getTaskDescription()) == 0) {
				searchResult = 1;
			}
